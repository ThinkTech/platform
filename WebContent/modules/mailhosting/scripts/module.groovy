...
class Service extends ActionSupport {
    ...
    def order(order){
       ...
       if(order.plan == "free"){
           ...
           order.year = 1
           params = [order.domain,order.extension,order.plan,order.price,order.year,order.action,order.eppCode,user.structure_id,true,order.email,"in progress"]
		...
		def text = '''\
		 ...
		 div(style : "padding-bottom:12px;margin-left:auto;margin-right:auto;width:80%;background:#fff") {
		    ...
		    div(style : "width:90%;margin:auto;margin-top : 30px;margin-bottom:30px") {
		     ...
		     p("Vous devez maintenant effectuer le paiement de vos factures pour la configuration de votre business email.")
		    }

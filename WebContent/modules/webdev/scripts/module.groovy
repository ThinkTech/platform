    ...
    def order(order) {
         ...
         if(!order.domainCreated){
	         ...
	         result = connection.executeInsert 'insert into projects(subject,priority,service,plan,description,user_id,structure_id,domain_id) values (?,?,?,?,?,?,?,?)', params
	          ...
	          connection.executeUpdate "update domains set plan = if(emailOn = false, 'free', plan), status = if(emailOn = 'in progress',?,status), email = if(emailOn = false,?,email), emailOn = if(emailOn = false,true, emailOn) where id = ?", [email,order.domain_id]
